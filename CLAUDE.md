...
- d3-hierarchy + d3-zoom + d3-transition for the infinite-canvas map (`src/components/TreeCanvas.tsx`): couples share a node, orthogonal connectors, minimap, `zoomTo` glide. Branches fold: a − toggle under every couple with children, a `+N` pill while folded (N hidden relatives), folds kept in localStorage `sft-collapsed:<treeId>`, the toggled couple stays put on screen, `zoomTo` unfolds the path to a searched person, and the zoom controls carry fold-all / unfold-all. A spouse's own ancestors (e.g. Teresa's Dusun line) are stacked above the spouse's card at negative depths rather than drawn as a separate tree; the founding couple is the root candidate with the most children. `PersonSpotlight.tsx` is the bottom-left card shown on select.
